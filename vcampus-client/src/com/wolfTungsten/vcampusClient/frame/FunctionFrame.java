...
public class FunctionFrame extends JFrame implements MouseListener {
	...
	public FunctionFrame(String Token) {
		...
		/*
		 * //虚假的空白标签 label_bank_exit= new JLabel();
...
//		panel_right.add("shop_4", panel_shop_history);
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getClickCount() == 1) {
			if (e.getSource() == label_info_main) {
				...
				HideAllMessagePanel();
				/*
				 * cardLayout.show(panel_right, "1"); panel_message_info.setVisible(true);
				 * panel_message_jwc.setVisible(false); panel_message_lib.setVisible(false);
				 * panel_message_shop.setVisible(false); panel_message_bank.setVisible(false);
				 * contentPane.setLayer(panel_message_info,new
				 * Integer(11));//将panel_message_info放置在比10层高的地方
				 */
				...
				HideAllMessagePanel();
					...
					HashMap<String, Object> record = shophistory(token);
					String[][]tablevalue = (String[][]) record.get("tablevalue");
					String name = (String) record.get("username");
					String cardnum = (String) record.get("cardnum");
					panel_shop_history = new ShoppingHistory(token,tablevalue,name,cardnum);
					panel_shop_history.setBackground(new Color(255, 255, 255));
					panel_right.add("shop_4", panel_shop_history);
	...
	public static HashMap<String, Object> borrowRecordrequest(String token) {
		...
		ArrayList<LinkedTreeMap<String, Object>> recordMaplist = (ArrayList<LinkedTreeMap<String, Object>>) response
				.getBody().get("recordMaplist");
	...
	public HashMap<String,Object> shophistory(String token){
		HashMap<String,Object> shoprecord = new HashMap<>();
		Client.Request request = new Request();
		request.setPath("shop/buyRecord");
		request.setToken(token);
		Response response = Client.fetch(request);
		...
		for(int i=0;i<row;i++) {
			...
			String recorduuid = (String) recordmap.get("uuid");
			String goodname = (String) recordmap.get("name");
			String amount = String.valueOf((int)(double)recordmap.get("amount"));
			String cost = String.valueOf((double)recordmap.get("cost"));
			long timestamp = (long)(double)recordmap.get("createtime");
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
			String date = df.format(new Date(timestamp*1000));
			tablevalue[i][0]=recorduuid;
			tablevalue[i][1]=goodname;
			tablevalue[i][2]=date;
			tablevalue[i][3] =amount;
			tablevalue[i][4] = cost;	
		}
		String name = (String) response.getBody().get("username");
		String cardnum =(String)response.getBody().get("cardnum");
		shoprecord.put("tablevalue", tablevalue);
		shoprecord.put("username", name);
		shoprecord.put("cardnum", cardnum);
		return shoprecord;
	}
