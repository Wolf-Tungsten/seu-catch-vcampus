...
public class DateChooserForReg extends JPanel {
	...
	public static DateChooserForReg getInstance() {
		return new DateChooserForReg();
	}
 
	public static DateChooserForReg getInstance(Date date) {
		return new DateChooserForReg(date);
	}
 
	public static DateChooserForReg getInstance(String format) {
		return new DateChooserForReg(format);
	}
 
	public static DateChooserForReg getInstance(Date date, String format) {
		return new DateChooserForReg(date, format);
	}
 
	/** 
	 * Creates a new instance of DateChooser 
	 */
	private DateChooserForReg() {
		this(new Date());
	}
 
	private DateChooserForReg(Date date) {
		this(date, "yyyy年MM月dd日");
	}
 
	private DateChooserForReg(String format) {
		this(new Date(), format);
	}
 
	private DateChooserForReg(Date date, String format) {
		initDate = date;
		...
		showDate.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent me) {
				if (showDate.isEnabled()) {
					...
					showDate.setForeground(new Color(59,120,103));//绿油油的
		...
		private void initJP1() {
			...
			yearleft.addMouseListener(new MouseAdapter() {
				public void mouseEntered(MouseEvent me) {
					...
					yearleft.setForeground(new Color(59,120,103));//绿油油
				}
			...
			yearright.addMouseListener(new MouseAdapter() {
				public void mouseEntered(MouseEvent me) {
					...
					yearright.setForeground(new Color(59,120,103));//绿油油
				}
			...
			monthleft.addMouseListener(new MouseAdapter() {
				public void mouseEntered(MouseEvent me) {
					...
					monthleft.setForeground(new Color(59,120,103));//绿油油
				}
			...
			monthright.addMouseListener(new MouseAdapter() {
				public void mouseEntered(MouseEvent me) {
					...
					monthright.setForeground(new Color(59,120,103));//绿油油
				}
	...
	private class MyLabel extends JLabel implements Comparator<MyLabel>, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			...
			if (year == now.get(Calendar.YEAR) && month == now.get(Calendar.MONTH) && day == now.get(Calendar.DAY_OF_MONTH)) {
				...
				gd.setColor(new Color(59,120,103));//绿油油
